 ...
 *
 * crypto.randomUUID exists only over HTTPS or on localhost, and this panel served over plain HTTP is
 * an ordinary way to run it locally. There it is
...
export function threadKey(userKey: string): string {
	return `home-lab.agent.thread.${userKey}`;
}
