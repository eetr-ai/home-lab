 ...
 * the one *before* the agent has to shorten the conversation, and nothing else
 * says it is coming.
 *
...
export default function ContextGauge({ gauge }: { gauge: Gauge }) {
	...
	return (
		...
		>
			<div
				className="h-1 w-10 overflow-hidden rounded-full bg-surface-sunken"
				role="progressbar"
			...
			>
				<div
					className={`h-full ${filled >= TIGHT ? "bg-warning-fg" : "bg-border-strong"}`}
					style={{ width: `${percent}%` }}
				/>
			</div>
