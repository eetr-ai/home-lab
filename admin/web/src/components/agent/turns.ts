...
const GUARDRAIL_NOTES: Record<string, string> = {
	...
	"exceeded max iterations":
		"It ran out of steps before finishing. Narrow the question, or raise AGENT_MAX_ITERATIONS on the deployment.",
};
...
export const HANDED_OVER_NOTE =
	"It was already working on this conversation, so your message joined that run " +
...
export function reduce(turn: Turn, event: AgentEvent): Turn {
	...
	switch (event.type) {
		...
		case "guardrail":
			return { ...turn, note: GUARDRAIL_NOTES[event.reason ?? ""] ?? "It stopped short of an answer." };
	}
}
