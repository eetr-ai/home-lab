...
//
// Verified is all it requires. There is no per-route permission here and that is
// deliberate — see auth.Middleware — so any token this API accepts can call any
// of these, reads and writes alike.
//
// Each route is registered with its literal path in the HandleFunc call, which
// is what the OpenAPI coverage test reads: it scans this source rather than the
// running mux, so a route assembled from pieces would be invisible to it.
func (h *Handler) Register(mux *http.ServeMux) {
	mux.HandleFunc("GET /api/helm/releases",
		h.listReleases)
	mux.HandleFunc("GET /api/helm/namespaces/{namespace}/releases",
		h.listNamespaceReleases)
	...
	mux.HandleFunc("GET /api/helm/namespaces/{namespace}/releases/{release}/history",
		h.readHistory)
	mux.HandleFunc("GET /api/helm/chart-versions", h.listChartVersions)
